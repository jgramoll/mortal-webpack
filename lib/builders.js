...
const b = {
  ...
  /**
   * Specify the ["mode"](https://webpack.github.io/docs/configuration.html#mode)
   * to use in the resulting bundle.
   *
   * @param  {String} mode
   *         One of the valid modes webpack understands.
   *          - 'development'
   *          - 'production'
   *          - 'none'
   *
  ...
  /**
   * Apply directives _unless_ a predicate holds true. This is the complement
   * of [[.when]].
   *
   * @param  {Union.<Boolean|function(): Boolean>} predicate
   * @param  {Array.<Directive>} directives
   ...
   *     ])
   */
  unless(predicate, directives) {
    if (typeof predicate === 'function') {
      return b.when(R.complement(predicate), directives);
    }
    else {
      return b.when(R.not(predicate), directives);
    }
  },
  ...
  /**
   * Minimize the resulting bundles
   *
   * @return {Directive}
   */
  optimizeJS() {
    return { name: 'optimize-js' }
  },
