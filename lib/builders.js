 ...
 * @alias FileExtension
 *
 * A string representing a file extension with the leading dot.
 * An "empty" file extension is required for webpack1 in order to
 * resolve files with any extension.
 *
 * Example values:
 *
 *     ""
...
const b = {
   ...
   *        This parameter will be overridden in case you're [[defining a DLL | .defineDLL]].
   *
   * @return {Directive}
   */
  output(options) {
   ...
   *
   * @param  {Array.<FilePattern>} options.dynamicDirectories
   ...
   *       'foo': 'bar',
   *
   *       // turn require("react/addons") into require("react-addons-test-utils")
   *       // but this won't apply to `require("react/addons/x")`
  ...
  /**
   * Define runtime constants that can be used by modules in the bundle. Popular
   * examples are exposing `process.env.NODE_ENV` to branch between
  ...
  compileHTML(options) {
    ...
    try {
      hasHtmlPlugin = require('html-webpack-plugin')
    }
    catch (e) {
      hasHtmlPlugin = false
    }
   ...
   * other bundles.
   *
   * Commonly this is referred to as the "vendor" or "commons" chunk.
   *
   ...
   *     c.generateBundle({ name: 'guest', modules: [ './src/guest/index.js' ] })
   *     c.generateBundle({ name: 'member', modules: [ './src/member/index.js' ] })
   *     c.generateBundle({ name: 'support', modules: [ './src/support/index.js' ] })
   *
   * > **Note**
   * >
   * > It makes sense to only have one common bundle in a build, and mortal-webpack
   * > currently performs no checks to ensure this is the case, so act with
   ...
   *     })
   *
   *     // in your source module, the following two statements are equivalent:
   ...
   * @param {Object} options
   *
   * @param {Union.<String|RegExp>} options.pattern
  ...
  /**
   * Helper for applying directives in a specific environment. This is
   * equivalent to calling [[.when]] with a predicate against `process.env.NODE_ENV`.
   *
   * @param  {String} env
  ...
  /**
   * Add coverage instrumenting using [istanbul](http://gotwarlost.github.io/istanbul/).
   *
   * The parameters are akin to those required by [[.compile]] since this
   * ends up generating a loader as well (or well, something like that at
   * least.)
   *
  ...
  /**
   * Define a bundle that can be used as a DLL.
   *
   * @param {String} options.path
   ...
   *     })
   */
  defineDLL({ path, name, libraryName = null, modules }) {
    return [
  ...
  /**
   * Set the context from which modules will be resolved (this also applies
   * to DLLs unless explicitly overridden.)
   *
   * When unset, webpack defaults to `process.cwd()` which may not be a good
   * idea.
   *
  ...
  /**
   * Break the build if a condition does not hold. This is very useful to ensure
   ...
   * @example
   *
   * Ensure that `COVERAGE=` is either not set or if it is, the value is valid:
   *
   *     b.invariant(!process.env.COVERAGE || process.env.COVERAGE === '1',
   *       "COVERAGE environment variable must either be unset or set to \"1\"."
   *     )
   *
   */
