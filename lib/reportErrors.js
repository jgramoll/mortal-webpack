...
const { name: ourName } = require('../package.json')
...
function dumpErrors(config, output) {
  return columnify(output.errors.map(error => printError(config, error)), {
...
function printError({ context, traceFileBlacklist }, { message, location }) {
  const trace = stackTrace.parse(location)
  ...
  return {
    error: message,
    location: `${omitContext(callerTrace.getFileName())}:${callerTrace.getLineNumber()}`
  }
}
