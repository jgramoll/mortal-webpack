...
const { assign, curry, flatten, flow, last } = require('./utils')
 ...
 *
 * @return {Object}
 *         Configuration object that can be passed to webpack.
 */
...
const composeOutput = directives => {
  ...
  const setLibraryNameForDLL = object => {
    ...
    if (dllDefinition) {
      return assign(object, { library: dllDefinition.libraryName })
    }
    else {
      return object
    }
