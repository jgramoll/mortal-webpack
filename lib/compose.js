...
const { assign, flow, last } = require('./utils')

module.exports = params => directives => compose(params)(flatten(directives))

const flatten = x => x.reduce(function(list, a) { return list.concat(a) }, [])
const compose = ({ happyPackOptions }) => directives => {
  checkInvariants(directives)
...
const checkInvariants = directives => {
  ...
  directives.filter(byName('invariant')).map(getProps).forEach(({ predicate, message }) => {
    if (!predicate) {
...
const composeOutput = directives => {
  const acceptOutput = () => directives.filter(byName('output')).map(getProps).reduce(function(_, d) {
    return assign({}, d)
  }, null)
  ...
  return flow([ acceptOutput, enableDevServer ])({})
}
...
const composeResolve = directives => {
  ...
  return {
    ...
    alias: directives.filter(byName('alias')).map(getProps).reduce(function(map, d) {
      return assign(map, d)
    }, {})
  }
}
...
const composeResolveLoader = directives => {
  ...
  return {
    ...
    alias: directives.filter(byName('alias-loader')).map(getProps).reduce(function(map, d) {
      return assign(map, d)
    }, {})
  }
}
...
const composeWatch = directives => {
  return directives.some(byName('watch'))
}
